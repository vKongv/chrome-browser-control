...
- **Dialogs / overlays:** compact snapshots scope to a visible modal (`aria-modal="true"` or `<dialog>` shown with `showModal()`). Other visible `role="dialog"` nodes are included in the current scope and do not steal it. Hidden or closed dialogs have no effect. To see the page behind a modal, pass `scope: "document"` or `scope: "main"`. To hide dialogs again, pass `ignoreRoles: ["dialog"]`. `ignoreRoles: []` includes dialogs in the current scope without changing the modal auto-scope. `mode: "full"` is the unscoped legacy snapshot. Target a dialog inside an iframe with `query_elements` / `list_frames` — a top-document snapshot does not see iframe modals.
