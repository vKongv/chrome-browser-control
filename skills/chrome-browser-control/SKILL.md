...
4. Act from fresh state.
   - Click/type using refs from a recent snapshot or query result. If click, type, `click_at`, or `keypress` fail with `DOCUMENT_HIDDEN`, call `activate_tab` and retry immediately. Read `visible` and `visibilityState` from the result. When `visible` is false, also read `reason`. Pass `allowHidden: true` only when `visibilityState` is `hidden` and you still need the write, or when you intentionally need background-tab writes. If `reason` is `host_permission_denied` or `document_unavailable`, grant host permission or reload; `allowHidden` will not help.
