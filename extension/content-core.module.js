...
export function toTabViewportPoint(x, y, documentRef = document) {
  let px = Number(x);
  let py = Number(y);
...
export function prepareTrustedClick({ ref, allowHidden = false } = {}, documentRef = document) {
  assertDocumentVisible(documentRef, allowHidden);
  const element = findByRef(ref, documentRef);
  if (!element) throw new Error(`No element found for ref ${ref}. Refresh snapshot and try again.`);
  element.scrollIntoView?.({ block: 'center', inline: 'center' });
...
export function prepareTrustedType({ ref, text, force = false, allowHidden = false } = {}, documentRef = document) {
  assertDocumentVisible(documentRef, allowHidden);
  const element = findByRef(ref, documentRef);
  if (!element) throw new Error(`No element found for ref ${ref}. Refresh snapshot and try again.`);
  if (isPasswordLike(element) && !force) {
...
export function prepareTrustedClickAt({ x, y, allowHidden = false } = {}, documentRef = document) {
  assertDocumentVisible(documentRef, allowHidden);
  if (typeof x !== 'number' || typeof y !== 'number') throw new Error('click_at requires numeric x and y');
...
export function prepareTrustedKeypress({ keys, allowHidden = false } = {}, documentRef = document) {
  assertDocumentVisible(documentRef, allowHidden);
  const keyList = Array.isArray(keys) ? keys : [keys];
  if (!keyList.length || keyList.some((key) => !key)) throw new Error('keypress requires keys');
