...
function cleanupRefStore(documentRef = document, now = nowMs()) {
...
function boundsForRef(ref, documentRef = document) {
...
function buildVisibleSnapshotFromDocument(documentRef = document, options = {}) {
  const windowRef = documentRef.defaultView || window;
...
function findByRef(ref, documentRef = document) {
...
function performScroll({ deltaX = 0, deltaY = 600, x, y } = {}, windowRef = window) {
...
function queryElements(options = {}, documentRef = document) {
  const now = typeof options?.now === 'number' ? options.now : nowMs();
  cleanupRefStore(documentRef, now);
  const windowRef = documentRef.defaultView || window;
...
function extractElements(options = {}, documentRef = document) {
...
function probeWaitCondition(options = {}, documentRef = document) {
  const result = {};
...
function installConsoleCapture(windowRef = window) {
...
function getConsoleLogs({ levels, limit } = {}) {
...
async function collectScroll(options = {}, documentRef = document, windowRef = documentRef.defaultView || window) {
