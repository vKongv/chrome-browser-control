...
- `activate_tab`: focuses an allowed tab and its Chrome window without changing the URL. Use this when click/type fail with `DOCUMENT_HIDDEN`, or when the tab is already active in an unfocused window. `screenshot.activated` only reports whether the tab was made active in its window — not whether the window was focused.
...
- `perform_actions`: runs up to 10 sequential page actions (`click`, `type`, `scroll`, `keypress`) in one broker round-trip. Fail-fast on the first step error; terminal `after` observations run only when every step succeeds. Click and type steps fail with `DOCUMENT_HIDDEN` on hidden documents unless that step sets `allowHidden: true`. Coordinate clicks stay on single-tool `click_at`. Steps cannot carry `after`, `tabId`, or `sessionTabId`.
