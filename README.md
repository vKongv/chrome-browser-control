...
- `navigate`: navigates the active tab or a specified `tabId` to an allowed URL, then waits for the tab to finish loading when possible. Defaults to `active: false` (no focus steal); pass `active: true` only when the tab must become visible. If loading times out, the result includes `pending: true` and a `warning`. Supports `after` observations after the load wait.
