...
- Navigate leaves focus alone by default (does not activate a background tab and does not deactivate the focused tab); pass `navigate({ active: true })` only when the tab must become visible. Use `activate_tab` to raise a tab and its window without navigating. It waits (bounded) for `document.visibilityState === "visible"` and returns `visibilityState` / `visible` / `reason`; do not treat `focused` as success.
