...
- Trusted CDP input is opt-in. Grant the optional `debugger` permission in the popup, `claim_tab`, then `cdp_attach({ sessionTabId })`. While attached, click/type/keypress/`click_at` and matching `perform_actions` steps use CDP. Detach with `cdp_detach`, or automatically on `release_tab` / `finalize_tabs` / 10-minute TTL. Typed errors: `CDP_PERMISSION_DENIED`, `CDP_ATTACH_REFUSED`, `CDP_DETACHED_BY_USER`, `CDP_ORIGIN_NOT_ALLOWED`, `CDP_TAB_NOT_CLAIMED`, `CDP_METHOD_NOT_PERMITTED`. Chrome shows a debugging banner on attached tabs. The MCP adapter does not open a raw CDP socket.
