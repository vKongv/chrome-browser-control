...
- Advisory claims remain default. Use `claim_tab({ exclusive: true, ttlMs?, owner? })` for fail-fast tab leases across parallel agents; MCP adapter injects `ownerId` per process. `ttlMs` (default 5 minutes) is idle time: each call resolved through the claim (`sessionTabId` or the current claim) extends the lease by its `ttlMs`. An expired claim's `sessionTabId` fails with a `TAB_CLAIM_EXPIRED` JSON error (`expiredAt`, `tabId`) instead of `No claimed tab`.
