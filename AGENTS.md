...
- Snapshot text carries inline refs for interactive elements in the `elements` list, at the spot where each sits: links as `[text](ref=h3)`, other controls as `[role "label" ref=h4]` or `[role ref=h5]`. Refs in text and list are the same handles. Inline refs cover only the returned text (`textLimit`); the list stays complete. `wait_for` text matching uses plain text without refs. Invisible (`visibility:hidden`) controls stay in the list but get no marker.
- The snapshot element list drops non-interactive elements inside `aria-hidden="true"` (decorative icons) and reports `ariaHiddenOmitted`. Interactive elements under `aria-hidden` stay, because modal libraries hide the page behind a dialog that way. Interactive elements with no text are named from non-`aria-hidden` `<img alt>` inside them. `query_elements` / `extract_elements` stay unfiltered.
...
Expected benchmark target: compact snapshots should remain at least 50% smaller than full snapshots on the dense fixture. Last verified reduction was 82.4%.
